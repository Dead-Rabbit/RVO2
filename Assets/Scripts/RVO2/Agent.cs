    ...
    internal class Agent
    {
        ...
        internal void computeNewVelocity()
        {
            ...
            for (int i = 0; i < obstacleNeighbors_.Count; ++i)
            {
                 ...
                 * 检查障碍物的速度障碍是否已经被先前构建的障碍物ORCA线所照顾
                 */
                ...
                // s 为，Ob1到Agent的向量在 Ob1->Ob2 向量上的投影长度
                float s = -relativePosition1 * obstacleVector / RVOMath.absSq(obstacleVector);
                // s * obsVector 为投影向量, distSqLine 为Agent到Obstacle的距离
                float distSqLine = RVOMath.absSq(-relativePosition1 - s * obstacleVector);

                // Line 的Direction 为，当前 Agent 收力的切线方向
                Line line;

                // s < 0 时，可以认为点的映射点在线 O1->O2 的超出O1的一侧
                if (s < 0.0f && distSq1 <= radiusSq)
                {
                    ...
                    if (obstacle1.convex_)
                    {
                        ...
                        // Line 的 Direction 取 -relativePosition1.y(), relativePosition1.x() 
                        // 考虑原因是：当前位置到目标点的位置的切线，同下
                ...
                // s > 1 时，可以认为点的映射点在线 O1->O2 的超出O2的一侧
                ...
                // s > 0, s < 1 时，可以认为点的映射点在线 O1->O2 的中间
         ...
         * constraint.
         * 解决指定线路上的一维线性程序，受线路和循环约束定义的线性约束
         * </summary>
         *
