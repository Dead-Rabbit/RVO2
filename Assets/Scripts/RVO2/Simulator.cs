...
namespace RVO
{
    ...
    public class Simulator
    {
        ...
        public float doStep()
        {
            if (workers_ == null)
            {
                ...
                for (int block = 0; block < workers_.Length; ++block)
                {
                    ...
                    workers_[block] = new Worker(block * getNumAgents() / workers_.Length, 
                        (block + 1) * getNumAgents() / workers_.Length, doneEvents_[block]);
                }
            }
         ...
         * 在寻找周围邻居的搜索距离，这个值设置过大，会让小球在很远 距离的时候做出避障行为，必须为非负数
         * </param>
         * 
         ...
         * are safe with respect to(至于，关于) other agents. The larger this number, the
         ...
         * 默认的最小时间量，在这个时间内，一个新的 Agent 的速度是由模拟计算出来的，
         * 这个速度是相对于其他 Agent 安全的。这个数字越大，Agent越早对其他Agent的存在作出反应，
         * 但Agent在选择其速度方面的自由度越小。必须是正数
         * Horizon: 范围，眼界
         * 
         * 代表计算动态的物体时的时间窗口
         * </param>
         ...
         * less freedom the agent has in choosing its velocities. Must be positive.
         * 默认的最小时间量，在这个时间内，新的Agent的速度是由模拟计算出来的，
         * 相对于障碍物来说是安全的。这个数字越大，Agent越早对障碍物的存在作出反应，
         * 但Agent在选择其速度方面的自由度越小。必须是正数
         * 
         * 代表计算静态的物体时的时间窗口，比如在RTS游戏中，小兵 向城墙移动时，没必要做出避障，
         * 这个值需要设置的很小
         * </param>
         * 
         * <param name="radius">
         * The default radius of a new agent. Must be non-negative.
         * 一个新的 Agent 的默认半径，必须为非负数
         *
         ...
         * 
         * <param name="velocity">
         * The default initial two-dimensional linear velocity of a new agent.
         * 一个新的 Agent 的默认2D速度
         *
         * 小球初始速度
         * </param>
